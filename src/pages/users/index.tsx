...
import { getUsers, removeUser } from "../../stores/thunk/userThunk";
    ...
    return (
        <>
            <Box sx={{ bgcolor: "#d6b4fc", p: 4 }}>
                {/* Search + Add User */}
                <SearchBar search={search} setSearch={setSearch} handleNewUser={handleNewUser} />

                {/* Data Table */}
                <DataTable users={filtered} open={open} handleClick={handleClick}
                    anchorEl={anchorEl}
            ...
            {/* Add Profile */}
            <DialogsBox open={newUser?.open} handleClose={handleNewClose} maxWidth="sm">
                <UserAdd header="Add Profile" user={newUser?.user} handleClose={handleNewClose} />
            </DialogsBox>

            {/* Edit Profile */}
            <DialogsBox open={isEditing?.open} handleClose={handleEditClose} maxWidth="sm">
                <UserEdit header="Edit Profile" user={isEditing?.user} handleClose={handleEditClose} />
            </DialogsBox>
